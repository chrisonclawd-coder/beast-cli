...
export class SwarmCoordinator {
     ...
     */
    async createTask(description, agent) {
    ...
    async runAll(taskIds) {
        ...
        for (let i = 0; i < tasksToRun.length; i += maxConcurrent) {
            ...
            const batchResults = await Promise.all(batch.map(task => this.runTask(task.id).catch(err => ({
                content: "",
                toolCalls: [],
                iterations: 0,
 ...
 */
export function createSwarmCoordinator(projectRoot, config) {
...
export function getSwarmCoordinator(projectRoot) {
