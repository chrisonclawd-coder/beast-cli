 ...
 */
export function registerCommands(program: Command, context?: Partial<CLIContext>): void {
  const defaultContext: CLIContext = {
    config: {} as BeastConfig,
  ...
  program
    ...
    .action(async (path, options) => {
      ...
      if (options.plan) console.log("  Mode: PLAN (read-only)")
      if (options.swarm) console.log("  Mode: SWARM (parallel)")
      if (options.voice) console.log("  Feature: Voice input enabled")
      if (options.vim) console.log("  Feature: Vim mode enabled")
      if (options.compact) console.log("  Feature: Auto-compaction enabled")
